...
int findOverlap(string s1, string s2, string &str){
    ...
    for(int i=1; i<=min(len1,len2); i++){
        if(s1.substr(len1-i) == s2.substr(0,i)){
            if(mx<i){
                mx=i; str = s1+s2.substr(i);
            }
        }
    }
    for(int i=1; i<=min(len1,len2); i++){
        if(s2.substr(len2-i) == s1.substr(0,i)){
            if(mx<i){
                mx=i; str = s2+s1.substr(i);
            }
        }
    }
    return mx;
...
string findSuperStr(string s[], int len){
    while(len != 1){
        int mx = -1, L = 0, R = 0;
        string resStr;
        for(int i=0; i<len; i++){
            for(int j=i+1; j<len; j++){
                string str;
                int overlaped = findOverlap(s[i],s[j],str);
                if(overlaped>mx){
                    mx = overlaped;
                    L = i; R = j;
                    resStr.assign(str);
